...
from aiogram.types import User # Можно удалить, если не используется, но оставляю для совместимости
...
async def upsert_user(uid: int, username: str | None, new_balance: int, *, registered_at: Optional[datetime] = None):
    """
    Обновляет или вставляет пользователя. 
    new_balance - это полный, актуальный баланс пользователя.
    registered_at - должен передаваться как ИМЕНОВАННЫЙ аргумент (keyword-only).
    """
    if not pool: return
    async with pool.acquire() as db:
        await db.execute("""
